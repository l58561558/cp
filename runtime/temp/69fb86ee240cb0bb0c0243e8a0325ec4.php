<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:57:"D:\phpStudy\WWW\cp/application/adminz\view\node\edit.html";i:1513665630;s:59:"D:\phpStudy\WWW\cp/application/adminz\view\public\base.html";i:1539586792;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
            ...
            <div class="col-md-12">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">编辑节点信息</h3>
                        <div class="pull-right">
                            <a href="<?php echo url('node/index'); ?>" class="btn btn-link btn-sm"><i class="fa fa-angle-double-left"></i>&nbsp;返回列表</a>
                        </div>
                    </div>
                        ...
                        <div class="box-body" id="tab_base">
                            <div class="layui-form-item">
                                ...
                                <div class="layui-input-block">
                                    <select name="authority_id" lay-verify="required" class="layui-input">
                                        <?php if(is_array((isset($authority_list) && ($authority_list !== '')?$authority_list:array())) || (isset($authority_list) && ($authority_list !== '')?$authority_list:array()) instanceof \think\Collection || (isset($authority_list) && ($authority_list !== '')?$authority_list:array()) instanceof \think\Paginator): $i = 0; $__LIST__ = (isset($authority_list) && ($authority_list !== '')?$authority_list:array());if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                            <option value="<?php echo $vo['authority_id']; ?>"<?php echo $vo['authority_id']==$node['authority_id']?'selected':''; ?>><?php echo $vo['name']; ?></option>
                                            <?php if(is_array((isset($vo['child']) && ($vo['child'] !== '')?$vo['child']:array())) || (isset($vo['child']) && ($vo['child'] !== '')?$vo['child']:array()) instanceof \think\Collection || (isset($vo['child']) && ($vo['child'] !== '')?$vo['child']:array()) instanceof \think\Paginator): $i = 0; $__LIST__ = (isset($vo['child']) && ($vo['child'] !== '')?$vo['child']:array());if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i;?>
                                                <option value="<?php echo $vo2['authority_id']; ?>" <?php echo $vo2['authority_id']==$node['authority_id']?'selected':''; ?>><?php echo $vo2['name']; ?></option>
                            ...
                            <div class="layui-form-item">
                                ...
                                <div class="layui-input-block">
                                    <select name="pid" class="form-control">
                                        ...
                                        <?php if(is_array((isset($node_list) && ($node_list !== '')?$node_list:array())) || (isset($node_list) && ($node_list !== '')?$node_list:array()) instanceof \think\Collection || (isset($node_list) && ($node_list !== '')?$node_list:array()) instanceof \think\Paginator): $i = 0; $__LIST__ = (isset($node_list) && ($node_list !== '')?$node_list:array());if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                            <option value="<?php echo $vo['node_id']; ?>"<?php echo $vo['node_id']==$node['pid']?'selected':''; ?>><?php echo $vo['node_name']; ?></option>
                            ...
                            <div class="layui-form-item">
                                ...
                                <div class="layui-input-block">
                                    <input type="text" name="node_name" lay-verify="required" class="layui-input" placeholder="节点名称" value="<?php echo $node['node_name']; ?>">
                                </div>
                            </div>
                            <div class="layui-form-item">
                                <label class="layui-form-label">控制器名称</label>
                                <div class="layui-input-block">
                                    <input type="text" name="controller" lay-verify="required" class="layui-input" placeholder="控制器名称" value="<?php echo $node['controller']; ?>" >
                                </div>
                            </div>
                            <div class="layui-form-item">
                                <label class="layui-form-label">方法名称</label>
                                <div class="layui-input-block">
                                    <input type="text" name="action" lay-verify="required" class="layui-input" placeholder="方法名称" value="<?php echo $node['action']; ?>" >
                                </div>
                            </div>
                            <div class="layui-form-item">
                                <label class="layui-form-label">参数</label>
                                <div class="layui-input-block">
                                    <input type="text" name="parameter" lay-verify="required" class="layui-input" placeholder="参数" value="<?php echo $node['parameter']; ?>">
                                </div>
                            </div>
                            <div class="layui-form-item">
                                <label class="layui-form-label">排序</label>
                                <div class="layui-input-block">
                                    <input type="text" name="sort_order" lay-verify="required" class="layui-input" placeholder="排序" value="<?php echo $node['sort_order']; ?>">
                                </div>
                            </div>
                        </div></div>
                        <div class="form-group" style="margin-left: 12px">
                            <label>附加选项</label>
                            <label for="menu_yes" style="padding-left:10px;">
                                <input <?php echo !empty($node['is_menu'])?'checked':''; ?> type="checkbox" id="menu_yes" name="is_menu" value="1"  /> 左侧导航显示
                            </label>
                        </div>                        
                        <div class="form-group" style="margin-left: 12px">
                            <input type="hidden" name="node_id" value="<?php echo $node['node_id']; ?>">
                            <button type="submit" class="layui-btn">添加</button>
                            <label for="jump_list" style="margin-left: 15px;"><input checked type="checkbox" data-url="<?php echo url('node/index'); ?>" id="jump_list" value="1" >返回列表页</label>
                        </div>
                    </form>
                </div>
            </div>
